...
public class CurvedText : Text
{
	...
	{	
        ...
        float r = (invert) ? -radius : radius;

		for (int i = 0; i < stream.Count; i++)
		{
			UIVertex v = stream[i];

			float percentCircumference = ((invert)?-1:1)* v.position.x/circumference;
			Vector3 offset = Quaternion.Euler(0.0f,0.0f,-percentCircumference*360.0f)*Vector3.up;
			v.position = (offset*r*scaleFactor + offset*v.position.y );
			v.position += Vector3.down*r*scaleFactor;
